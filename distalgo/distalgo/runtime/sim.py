...
class DistProcess(multiprocessing.Process):
    ...
    def run(self):
        try:
            ...
            for p in self.send_threads: p.join()

            if self.before_termination is not None:
                self.before_termination(self.__class__.__name__)
            #            self._id.close()
    ...
    def spawn(self, pcls, args):
        ...
        if self.after_spawn is not None:
            self.after_spawn(cid, p.__class__.__name__)
        ...
        for e in self._event_backlog:
            ...
            for p in patterns:
                if (p.match(e)): # Match and handle
                    ...
                    for h in p.handlers:
                        if self.event_processed is not None:
                            self.event_processed(e)
                        h(*args)
                    ishandled = True
