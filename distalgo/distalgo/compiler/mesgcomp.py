...
class SentReceivedTransformer(NodeTransformer):
    ...
    def genHandlerFuncDef(self, name, vname, vlist):
        arglist = [arg("_"+str(i)+"_"+v, None) for (i, v) in vlist]
        ...
        m = Attribute(Name(vname, Load()),
                      ...
                      Load())
        if (len(vlist) > 0):
            a = Tuple([Name("_"+str(i)+"_"+v, Load()) for (i, v) in vlist], Load())
        else:
            a = Name("True", Load())
        ...
        if (len(free) == 0):    # No free variables
            ...
            for i, n in vlist:
                if n in free:
                    idx = i
                    break
            elt = Name(free[0]+"_"+str(idx), Load())
        else:
            elt = Tuple([Name(n+"_"+str(i), Load())
                         for (i, n) in vlist if n in free], Load())

        if (len(vlist) > 0):
            target = Tuple([Name(v+"_"+str(i), Store()) for (i, v) in vlist], Store())
        else:
            target = Name("v_", Load())
        iterator = Name(vname, Load())
        ifs = [Compare(Name(v+"_"+str(i), Load()), [Eq()], [Name(v, Load())])
               for (i, v) in vlist if v in bound]
